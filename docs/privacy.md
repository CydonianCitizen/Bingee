...
Reversible media links (Room v1 initial stable schema) store local association pairs between provider-qualified identities and write a local audit trail (`media_link_audit`). Audit events record only provider-qualified external identities (`source`, `media_type`, `external_id`), action types (`LINKED`, `UNLINKED`, `PREFERRED_PRESENTATION_CHANGED`), origin, and UTC timestamps. Audit records contain no titles, overview text, watch progress, ratings, search queries, credentials, or analytics.
