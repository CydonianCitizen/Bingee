...
Optional release notifications are disabled by default. Their lead time and movie/season/episode category choices use ordinary Preferences DataStore and may follow Android backup policy. Room v6 stores a minimal delivery identity plus deterministic notification ID and delivery timestamp so the same provider-aware event/date/lead configuration is not notified twice. It stores no rendered notification text, title, overview, watched state, rating, credential, or provider response. Delivery rows follow Room backup policy and may suppress duplicates after platform restore.
