...
internal class SeasonCacheFreshnessPolicy @Inject constructor(private val clock: Clock) {
    fun classify(fetchedAt: Instant): CacheFreshness {
        val now = clock.instant()
        if (fetchedAt.isAfter(now)) return CacheFreshness.STALE
        return if (Duration.between(fetchedAt, now) < MAX_AGE) CacheFreshness.FRESH else CacheFreshness.STALE
    }
